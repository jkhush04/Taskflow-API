...
/**
 * Runs once per request: extracts a Bearer token, validates it, and (if valid) populates the
 * SecurityContext so downstream @PreAuthorize checks and controller methods see an authenticated user.
 * No session is ever created - this is what makes the API fully stateless.
 */
@Component
